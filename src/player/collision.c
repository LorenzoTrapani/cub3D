...
/*   Updated: 2024/09/27 19:24:50 by lotrapan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_orizontal_collision(t_data *data, double new_x)
{
	...
	i = map_x;
	while (i != new_map_x + 1 * (new_x > data->player.x) - 1 * (new_x < data->player.x))
	{
...
int	check_vertical_collision(t_data *data, double new_y)
{
	...
	i = map_y;
	while (i != new_map_y + 1 * (new_y > data->player.y) - 1 * (new_y < data->player.y))
	{
